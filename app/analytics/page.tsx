  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-slate-800/80">
        ...
        <button
          onClick={fetchAll}
          className="flex items-center space-x-2 text-xs font-medium px-3.5 py-2 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-800 transition-all cursor-pointer"
        >
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-9 gap-4">
            {[
              { label: "Total Ads Tracked", value: totalAds.toLocaleString(), icon: BarChart3, color: "text-indigo-400", bg: "bg-indigo-500/10" },
              { label: "Average Results", value: stats ? stats.averageResults.toLocaleString() : "0", icon: BarChart3, color: "text-indigo-400", bg: "bg-indigo-500/10" },
              { label: "Highest Results", value: stats ? stats.highestResults.toLocaleString() : "0", icon: TrendingUp, color: "text-purple-400", bg: "bg-purple-500/10" },
              { label: "High Volume (50+)", value: highVolume.length, icon: Flame, color: "text-amber-400", bg: "bg-amber-500/10" },
              { label: "Zero Ad Pages", value: zeroAds.length, icon: AlertCircle, color: "text-slate-400", bg: "bg-slate-500/10" },
              { label: "Success Rate", value: `${pages.length > 0 ? Math.round((completed.length / pages.length) * 100) : 0}%`, icon: CheckCircle2, color: "text-emerald-400", bg: "bg-emerald-500/10" },
              { label: "Failed Pages", value: failed.length, icon: TrendingDown, color: "text-rose-400", bg: "bg-rose-500/10" },
              { label: "Unclear Pages", value: unclear.length, icon: ShieldAlert, color: "text-purple-400", bg: "bg-purple-500/10" },
              {
                label: "Last Import",
                ...
                <div key={i} className="glass-card rounded-xl p-3.5 flex flex-col justify-between transition-all hover:-translate-y-0.5 duration-200 min-h-[90px]">
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider truncate">{item.label}</span>
                      <div className={`p-1 rounded-lg ${item.bg}`}>
                        <Icon className={`w-3.5 h-3.5 ${item.color}`} />
                      </div>
                    </div>
                    <div className={`font-extrabold text-slate-100 truncate ${item.isText ? "text-xs mt-1 font-mono" : "text-lg"}`} title={String(item.value)}>
                      {item.value}
                    </div>
                  </div>
                  {item.subtitle && (
                    <div className="text-[10px] text-slate-500 mt-1 truncate">{item.subtitle}</div>
                  )}
                </div>
              );
            })}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 glass-card rounded-xl p-5">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <span className="text-sm font-bold text-slate-100">Top 10 Pages by Active Ads</span>
              </div>
                      ...
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center space-x-2">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-xs font-medium text-slate-200 hover:text-indigo-300 truncate max-w-[220px] transition-colors"
                          >
                            {p.displayName || "Meta Ad Search"}
                          </a>
                          {(p.currentResults ?? 0) >= 50 && (
                            <Flame className="w-3 h-3 text-amber-400 fill-amber-400/40 shrink-0" />
                          )}
                        </div>
                      </div>
            ...
            <div className="glass-card rounded-xl p-5">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <span className="text-sm font-bold text-slate-100">Status Distribution</span>
              </div>
              <div className="space-y-3">
                {statusData.map((item) => (
                  <div key={item.label}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="glass-card rounded-xl p-5">
              <div className="flex items-center space-x-2 mb-3">
                ...
                <span className="text-sm font-bold text-slate-100">Top Gainers (Since Last Scan)</span>
              </div>
              {gainers.length === 0 ? (
                ...
                <div className="space-y-2">
                  {gainers.map((p) => (
                    <div key={p.id} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-slate-800/40 transition-all">
                      <a href={p.url} target="_blank" rel="noreferrer" className="text-xs font-medium text-slate-200 hover:text-emerald-300 truncate max-w-[180px] transition-colors">
                        {p.displayName || "Meta Ad Search"}
                      </a>
                      <span className="inline-flex items-center text-xs font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20 shrink-0">
            ...
            <div className="glass-card rounded-xl p-5">
              <div className="flex items-center space-x-2 mb-3">
                ...
                <span className="text-sm font-bold text-slate-100">Top Decliners (Since Last Scan)</span>
              </div>
              {decliners.length === 0 ? (
                ...
                <div className="space-y-2">
                  {decliners.map((p) => (
                    <div key={p.id} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-slate-800/40 transition-all">
                      <a href={p.url} target="_blank" rel="noreferrer" className="text-xs font-medium text-slate-200 hover:text-rose-300 truncate max-w-[180px] transition-colors">
                        {p.displayName || "Meta Ad Search"}
                      </a>
                      <span className="inline-flex items-center text-xs font-bold text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20 shrink-0">
