...
export function SpyFilters({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/40 p-4 mb-4">
      {/* 1. Top Bar: Search Input, Filtered Count & Quick Controls */}
      <div className="flex flex-col sm:flex-row items-center gap-3 justify-between">
        {/* Search Bar & Result Count Pill */}
        <div className="flex items-center gap-2.5 w-full sm:w-auto flex-1 max-w-xl">
          ...
          {filteredCount !== undefined && (
            <div
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 shrink-0 whitespace-nowrap shadow-sm"
              title={`${filteredCount} ads match your active filters`}
            >
              <span className="w-2 h-2 rounded-full bg-indigo-500 shrink-0" />
              <span>
                <strong className="text-slate-900 dark:text-white font-bold">{filteredCount.toLocaleString()}</strong>{" "}
